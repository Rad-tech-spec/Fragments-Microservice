...
async function readFragment(ownerId, id) {
  ...
  try {
    ...
    throw new Error('There is no fragments data');
  }
}
...
async function deleteFragment(ownerId, id) {
  ...
  const command_db = new DeleteCommand(params_db);

  try {
    await sleep(100);
    await s3Client.send(command);
    await ddbDocClient.send(command_db);
    await sleep(100);
    return 'Deleted';
  } catch (err) {
    const { Bucket, Key } = params;
    logger.error({ err, Bucket, Key }, 'Error deleting fragment data from S3 or Memorydb');
    throw new Error('unable to read fragment data');
  }
}
